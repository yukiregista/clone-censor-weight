...
def load_scenario3_parameters():
    text = read_yaml_text("scenario3.yaml", __package__)
    ...
    raw[ScenarioVariables.CENS.name]["variable_normalizers"] = {
        ScenarioVariables.AGE: define_age_normalizer(raw)}
    # raw[ScenarioVariables.COMP.name]["variable_normalizers"] = {
        # ScenarioVariables.AGE: define_age_normalizer(raw)}
    return raw


def load_scenario3_params_and_variables():
    PARAMETERS = load_scenario3_parameters()
    LAG_CONFIGS = LagConfigs.get_scenario3_lags()
    AllVariables = [
        Variable(
        ...
        Variable(
            id=ScenarioVariables.CENS,
            value_type=ScenarioVariableTypes[ScenarioVariables.CENS],
            is_time_varying=True,
            parent_variables=LAG_CONFIGS[ScenarioVariables.CENS],
            CPD=CENS_CPD(**PARAMETERS[ScenarioVariables.CENS.name]),
        ),
        # Variable(
        #     id=ScenarioVariables.COMP,
        #     value_type=ScenarioVariableTypes[ScenarioVariables.COMP],
        #     is_time_varying=True,
        #     parent_variables=LAG_CONFIGS[ScenarioVariables.COMP],
        #     CPD=COMP_CPD(**PARAMETERS[ScenarioVariables.COMP.name])
        # )
    ]
    configs = {
